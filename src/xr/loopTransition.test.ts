...
describe('masked terrain loop transition', () => {
  it('stays transparent before the downhill trigger', () => {
    const result = stepLoopTransition(createLoopTransitionState(), 1, LOOP_TRIGGER_Z - 0.01)
  ...
  it('rebases once at full opacity and preserves overshoot', () => {
    const first = stepLoopTransition(
      { phase: 'fading-out', elapsed: LOOP_FADE_OUT_SECONDS - 0.01 },
      0.02,
      LOOP_TRIGGER_Z + 3,
    )
    ...
    const next = stepLoopTransition(first.state, 0, rebaseLoopCoordinate(LOOP_TRIGGER_Z + 3))
  ...
  it('fades back to a transparent idle state', () => {
    ...
    const finished = stepLoopTransition(middle.state, LOOP_FADE_IN_SECONDS, 0)
