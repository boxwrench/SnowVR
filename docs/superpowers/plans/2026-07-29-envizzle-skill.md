...
**Tech Stack:** Node 24 ESM, `node:test` + `node:assert` (built in, no test framework dependency), `pngjs` for PNG decode in tests and gates, `playwright` for the browser run (already the incumbent choice in `prompt template/verify_demo.mjs`).
...
- Only two runtime/dev dependencies permitted in `skill/package.json`: `pngjs` and `playwright`.
- Skill name is exactly `envizzle`. Invoked as `/envizzle`.
- Repo path `prompt template/skill/` is the source of truth. `~/.claude/skills/envizzle/` is a generated copy — never edit it directly.
...
- Every file created under `prompt template/skill/` uses LF line endings.
...
| Path | Responsibility |
|---|---|
| `prompt template/skill/SKILL.md` | Frontmatter + interview flow + assembly instructions Claude follows |
| `prompt template/skill/TEMPLATE.md` | Brief skeleton with `{{TOKEN}}` slots and `<!--SECTION:name-->` markers |
| `prompt template/skill/references/character-recipe.md` | The numeric humanoid construction spec, inlined verbatim |
| `prompt template/skill/lib/assemble.mjs` | Token substitution, section stripping, recipe inlining, CLI entry |
| `prompt template/skill/lib/color.mjs` | `relativeLuminance`, `saturation`, `hexToRgb01` — shared by coherence and gates |
| `prompt template/skill/lib/coherence.mjs` | Config → array of conflicts |
| `prompt template/skill/lib/presets/biomes.mjs` | Biome token bundles |
| `prompt template/skill/lib/presets/archetypes.mjs` | Character archetypes as rig parameters |
| `prompt template/skill/lib/presets/mechanics.mjs` | Centrepiece mechanic bundles |
| `prompt template/skill/lib/presets/cameras.mjs` | Camera/presentation modes |
| `prompt template/skill/lib/presets/optional-systems.mjs` | Optional system axes |
| `prompt template/skill/lib/presets/showcase.mjs` | 6 complete coherence-clean configs |
| `prompt template/skill/lib/ambition.mjs` | Ambition level → set of enabled section names |
| `prompt template/skill/verify/gates.mjs` | Pure pixel-buffer gate functions |
| `prompt template/skill/verify/verify_demo.mjs` | Playwright orchestrator invoking gates |
| `prompt template/skill/install.mjs` | Copy skill → `~/.claude/skills/envizzle/` |
| `prompt template/skill/tests/*.test.mjs` | All tests |
| `prompt template/skill/tests/fixtures/` | Real known-bad PNGs + synthetic generator |
...
**Files:**
- Create: `prompt template/skill/package.json`
- Create: `prompt template/skill/lib/assemble.mjs`
- Create: `prompt template/skill/TEMPLATE.md`
- Test: `prompt template/skill/tests/assemble.test.mjs`
...
Create `prompt template/skill/tests/assemble.test.mjs`:
...
Run: `cd "prompt template/skill" && node --test tests/assemble.test.mjs`
...
Create `prompt template/skill/TEMPLATE.md` by copying the current
`prompt template/TEMPLATE.md` and making exactly these changes:
...
```bash
git add "prompt template/skill/package.json" "prompt template/skill/lib/assemble.mjs" "prompt template/skill/TEMPLATE.md" "prompt template/skill/tests/assemble.test.mjs"
...
**Files:**
- Create: `prompt template/skill/references/character-recipe.md`
- Test: `prompt template/skill/tests/character-recipe.test.mjs`
...
Create `prompt template/skill/tests/character-recipe.test.mjs`:
...
Run: `cd "prompt template/skill" && node --test tests/character-recipe.test.mjs`
Expected: FAIL — `ENOENT` on `references/character-recipe.md`

- [ ] **Step 3: Write the recipe**

Create `prompt template/skill/references/character-recipe.md`. It must contain
...
```bash
git add "prompt template/skill/references/character-recipe.md" "prompt template/skill/tests/character-recipe.test.mjs"
...
**Files:**
- Create: `prompt template/skill/lib/color.mjs`
- Create: `prompt template/skill/lib/coherence.mjs`
- Test: `prompt template/skill/tests/coherence.test.mjs`
...
Create `prompt template/skill/tests/coherence.test.mjs`:
...
Run: `cd "prompt template/skill" && node --test tests/coherence.test.mjs`
...
```bash
git add "prompt template/skill/lib/color.mjs" "prompt template/skill/lib/coherence.mjs" "prompt template/skill/tests/coherence.test.mjs"
...
**Files:**
- Create: `prompt template/skill/lib/ambition.mjs`
- Create: `prompt template/skill/lib/presets/biomes.mjs`
- Test: `prompt template/skill/tests/presets-biomes.test.mjs`
...
Create `prompt template/skill/tests/presets-biomes.test.mjs`:
...
Run: `cd "prompt template/skill" && node --test tests/presets-biomes.test.mjs`
...
Create `prompt template/skill/lib/presets/biomes.mjs` with at least six biomes:
...
```bash
git add "prompt template/skill/lib/ambition.mjs" "prompt template/skill/lib/presets/biomes.mjs" "prompt template/skill/tests/presets-biomes.test.mjs"
...
**Files:**
- Create: `prompt template/skill/lib/presets/archetypes.mjs`
- Create: `prompt template/skill/lib/presets/mechanics.mjs`
- Create: `prompt template/skill/lib/presets/cameras.mjs`
- Create: `prompt template/skill/lib/presets/optional-systems.mjs`
- Test: `prompt template/skill/tests/presets-other.test.mjs`
...
Create `prompt template/skill/tests/presets-other.test.mjs`:
...
Run: `cd "prompt template/skill" && node --test tests/presets-other.test.mjs`
Expected: FAIL — cannot find `../lib/presets/archetypes.mjs`

- [ ] **Step 3: Implement archetypes**

Create `prompt template/skill/lib/presets/archetypes.mjs` with at least five:
...
```bash
git add "prompt template/skill/lib/presets/" "prompt template/skill/tests/presets-other.test.mjs"
...
**Files:**
- Create: `prompt template/skill/lib/presets/showcase.mjs`
- Modify: `prompt template/skill/lib/assemble.mjs` (add `buildConfig` and CLI)
- Test: `prompt template/skill/tests/showcase.test.mjs`
...
Create `prompt template/skill/tests/showcase.test.mjs`:
...
Run: `cd "prompt template/skill" && node --test tests/showcase.test.mjs`
Expected: FAIL — cannot find `../lib/presets/showcase.mjs`

- [ ] **Step 3: Implement the showcase configs**

Create `prompt template/skill/lib/presets/showcase.mjs`. Exactly six entries.
...
Append to `prompt template/skill/lib/assemble.mjs`:
...
Run: `cd "prompt template/skill" && node --test tests/`
Expected: PASS, all tests across all five test files

- [ ] **Step 6: Smoke-test the CLI by eye**

Run: `cd "prompt template/skill" && node lib/assemble.mjs alpineDawn --out /tmp/brief.md && head -60 /tmp/brief.md`
Expected: a clean brief with no `{{` and no `SECTION` markers.

- [ ] **Step 7: Commit**

```bash
git add "prompt template/skill/lib/presets/showcase.mjs" "prompt template/skill/lib/assemble.mjs" "prompt template/skill/tests/showcase.test.mjs"
...
**Files:**
- Create: `prompt template/skill/verify/gates.mjs`
- Create: `prompt template/skill/tests/fixtures/make-synthetic.mjs`
- Create: `prompt template/skill/tests/gates.test.mjs`
- Copy: `prompt template/skill/tests/fixtures/real-black-frame.png` (from the reference output)
- Copy: `prompt template/skill/tests/fixtures/real-idle-frame.png`
...
Run: `cd "prompt template/skill" && node --test tests/gates.test.mjs`
...
```bash
git add "prompt template/skill/verify/gates.mjs" "prompt template/skill/tests/gates.test.mjs" "prompt template/skill/tests/fixtures/" "prompt template/skill/package-lock.json"
...
**Files:**
- Create: `prompt template/skill/verify/verify_demo.mjs`
- Create: `prompt template/skill/verify/README.md`
...
The replacement for `prompt template/verify_demo.mjs`, whose line 133 logged
`PASS: Screenshots successfully saved` without inspecting any pixel.

- [ ] **Step 1: Write the orchestrator**

Create `prompt template/skill/verify/verify_demo.mjs` implementing this flow.
...
Create `prompt template/skill/verify/README.md` documenting: how to run it, the
...
Run: `cd "prompt template/skill" && node verify/verify_demo.mjs "/c/Users/wests/OneDrive/Desktop/New folder (3)"`
...
```bash
git add "prompt template/skill/verify/verify_demo.mjs" "prompt template/skill/verify/README.md"
...
**Files:**
- Create: `prompt template/skill/SKILL.md`
- Test: `prompt template/skill/tests/skill-md.test.mjs`
...
Create `prompt template/skill/tests/skill-md.test.mjs`:
...
Run: `cd "prompt template/skill" && node --test tests/skill-md.test.mjs`
Expected: FAIL — `ENOENT` on `SKILL.md`

- [ ] **Step 3: Write SKILL.md**

Create `prompt template/skill/SKILL.md` with this frontmatter and structure:
...
```bash
git add "prompt template/skill/SKILL.md" "prompt template/skill/tests/skill-md.test.mjs"
...
**Files:**
- Create: `prompt template/skill/install.mjs`
- Test: `prompt template/skill/tests/install.test.mjs`
...
Create `prompt template/skill/tests/install.test.mjs`:
...
test('excludes tests, node_modules, and the lockfile', () => {
...
Run: `cd "prompt template/skill" && node --test tests/install.test.mjs`
...
const HERE = path.dirname(fileURLToPath(import.meta.url));
const EXCLUDE = [/^tests[\\/]/, /node_modules/, /package-lock\.json$/, /^\.git/];
...
Run: `cd "prompt template/skill" && node install.mjs && ls ~/.claude/skills/envizzle/`
Expected: `SKILL.md`, `TEMPLATE.md`, `references/`, `lib/`, `verify/`, `install.mjs`

- [ ] **Step 6: Commit**

```bash
git add "prompt template/skill/install.mjs" "prompt template/skill/tests/install.test.mjs"
git commit -m "feat(envizzle): add installer to ~/.claude/skills"
```

---

### Task 11: Retire the superseded files

**Files:**
- Delete: `prompt template/BIOME_TECHDEMO_TEMPLATE.md`
- Delete: `prompt template/TEMPLATE_GUIDE.md`
- Delete: `prompt template/TEMPLATE.md`
- Delete: `prompt template/og prompt.txt`
- Delete: `prompt template/verify_demo.mjs`
- Create: `prompt template/README.md`
- Keep untouched: `prompt template/prompt_builder.html`

Deletion is last because Tasks 2, 4, and 5 mine these files for content. Do not
start this task until the full suite passes.

- [ ] **Step 1: Confirm the whole suite passes before deleting anything**

Run: `cd "prompt template/skill" && node --test tests/`
Expected: PASS across all seven test files. If anything fails, stop — the old
files are still the only copy of some content.

- [ ] **Step 2: Confirm the content was actually mined**

Check each of these before deleting its source:

| Old file | Content must now live in |
|---|---|
| `BIOME_TECHDEMO_TEMPLATE.md` §2.6 guidance | `references/character-recipe.md` |
| `BIOME_TECHDEMO_TEMPLATE.md` biome examples | `lib/presets/biomes.mjs` |
| `TEMPLATE_GUIDE.md` paradigm comparison | `lib/coherence.mjs` rules |
| `TEMPLATE_GUIDE.md` architectural checklist | `TEMPLATE.md` acceptance criteria |
| `TEMPLATE.md` token set | `skill/TEMPLATE.md` |
| `verify_demo.mjs` structure audit + build check | `verify/verify_demo.mjs` |

Run a spot check that no biome content was lost:
`grep -c "amp" "prompt template/skill/lib/presets/biomes.mjs"`
...
Source of truth for the `envizzle` skill.

- `skill/` — the skill. Edit here, then run `node skill/install.mjs` to update
  the generated copy at `~/.claude/skills/envizzle/`.
- `prompt_builder.html` — standalone manual form, kept as an alternative to the
  skill interview. Not wired to the preset libraries.

The earlier `TEMPLATE.md`, `BIOME_TECHDEMO_TEMPLATE.md`, `TEMPLATE_GUIDE.md`,
`og prompt.txt`, and `verify_demo.mjs` were superseded by `skill/` and removed in
this commit. Recover any of them with:

    git log --diff-filter=D --name-only -- "prompt template/"
    git show <commit>^:"prompt template/<file>"
```
...
```bash
cd "C:/GitHub/SnowVR"
git rm "prompt template/BIOME_TECHDEMO_TEMPLATE.md" \
       "prompt template/TEMPLATE_GUIDE.md" \
       "prompt template/TEMPLATE.md" \
       "prompt template/og prompt.txt" \
       "prompt template/verify_demo.mjs"
```

- [ ] **Step 5: Confirm nothing referenced the deleted files**

Run: `cd "C:/GitHub/SnowVR" && grep -rn "BIOME_TECHDEMO_TEMPLATE\|TEMPLATE_GUIDE\|og prompt" --exclude-dir=node_modules --exclude-dir=.git . || echo "no dangling references"`
Expected: `no dangling references`, or only hits inside `docs/superpowers/`
(the spec and this plan legitimately discuss them by name).

- [ ] **Step 6: Run the suite one final time**

Run: `cd "prompt template/skill" && node --test tests/`
Expected: PASS. Nothing in `skill/` should depend on the deleted files.

- [ ] **Step 7: Commit**

```bash
git add "prompt template/README.md"
git commit -m "chore(envizzle): retire superseded templates and verify script

Content mined into skill/references/character-recipe.md,
skill/lib/presets/, skill/lib/coherence.mjs, and skill/verify/.
Recoverable from git history; README documents how."
```
...
| `prompt_builder.html` unchanged | 11 (explicitly kept) |
